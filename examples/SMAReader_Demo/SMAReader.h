...
class SMAReader {
  public:
     ...
     /* Read all available keys/values
      * returns: true if success (after the specified number of tries) else false
      * Note that it does not return the keys/values
      * Used for debugging purposes and to figure out which keys/values are available
      */
     bool getAllValues();
